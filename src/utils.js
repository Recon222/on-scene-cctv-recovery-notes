...
export const generateNotes = (formData) => {
  ...
  if (formData.address && formData.onSceneArrival) {
    const arrivalTime = DateTime.fromISO(formData.onSceneArrival).toFormat('yyyy-MM-dd @ HHmm\\h\\r\\s');
  ...
  if (formData.extractFrom && formData.extractTo && formData.timeDifference) {
    const actualFromTime = DateTime.fromISO(formData.extractFrom).toFormat('yyyy-MM-dd HHmm\\h\\r\\s');
    const actualToTime = DateTime.fromISO(formData.extractTo).toFormat('HHmm\\h\\r\\s');
    const dvrFromTime = DateTime.fromISO(formData.correctedDvrFrom).toFormat('yyyy-MM-dd HHmm\\h\\r\\s');
    const dvrToTime = DateTime.fromISO(formData.correctedDvrTo).toFormat('HHmm\\h\\r\\s');

    if (formData.timeDifference.formatted !== '00:00:00') {
      notes.push(
        `• Recovered requested cameras from ${actualFromTime} to ${actualToTime} (actual time)/ ${dvrFromTime} to ${dvrToTime} (DVR time)`,
        `- Time offset: DVR is ${formData.timeDifference.formatted} ${formData.timeDifference.direction} real time.`
      );
    } else {
      notes.push(
        `• Recovered requested cameras from ${actualFromTime} to ${actualToTime} (DVR/actual time) [No offset]`
      );
    }
  }
